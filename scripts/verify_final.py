...
1. 숫자 정합성 — 집계 원본에서 다시 계산한 표기가 화면 글에 그대로 있는지.
2. 인사이트 수치 전수 대조 — 인사이트 문장의 모든 숫자가 허용 목록 안인지.
3. 금지 표현 스캔 — 빌드 산출물과 화면 글에 화면에 쓰지 않는 표현이 없는지.
...
FORBIDDEN = ["—", "–", "ㅡ", "§", "n=", "capped", "uncapped",
             ...
             "가중 재추정"]
FORBIDDEN_NAME = {"—": "줄표", "–": "짧은 줄표", "ㅡ": "낱자 으",
                  "§": "절 기호"}
...
def check_numbers(text):
    ...
    raid_pct = sum(fame[b]["pct"] for b in RAID_BINS)
    expect["레이드 진입 구간 비중"] = pct(raid_pct)
    ...
    expect["가장 높은 직업과 낮은 직업 배수"] = f"{best[3] / worst[3]:.2f}배"
    ...
    ok(sum(b["count"] for b in d["fame_bins"]) == meta["sample_size"] - meta["fame_missing"],
       "명성 구간 인원 합이 명성 보유 표본과 다릅니다")
    ok(sum(j["count"] for j in d["job"]) == meta["sample_size"], "직업 인원 합이 표본 크기와 다릅니다")
    ok(sum(s["count"] for s in d["stage"]) == meta["sample_size"], "각성 단계 인원 합이 표본 크기와 다릅니다")
    ok(sum(o["count"] for o in act["overall"]) == act["subsample_size"],
       "활성도 인원 합이 조사 인원과 다릅니다")
    ok(abs(sum(adj.values()) - 100) < 0.5, "보정 후 비중 합이 100 근처가 아닙니다")
...
def main():
    ...
    n_expect = check_numbers(rendered)
