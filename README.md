...
스스로 검증해서 결론을 뒤집은 사례가 둘 있습니다. 둘 다 처음 발표한 수치는
고치지 않고, 검증 결과를 나란히 두는 방식으로 리포트에 남겼습니다.

### 직업 순위에서 사라진 발견

처음에는 검색 방식에 따라 직업 구성이 크게 다르다는 결과가 나왔습니다.
그런데 직업 기준을 성장을 마친 캐릭터로 좁히자 그 차이가 거의 사라졌습니다.
커 보이던 차이의 상당 부분이 막 만들어 전직 중인 캐릭터가 섞여 있던 탓이었습니다.
발견 하나를 잃었지만 남은 수치는 더 믿을 만해졌습니다.

### 최대 편향이라 여겼던 것의 영향은 작았다

이 조사가 스스로 밝힌 가장 큰 한계는 검색이 200명에서 잘린다는 점이었고,
전체 검색의 45.8%가 잘렸습니다. 그래서 명성 점수로 직접 훑는 다른 방법으로
같은 것을 다시 재봤습니다. 요청 947번, 캐릭터 18,552명입니다.
...
성장 단계 분포는 거의 같았습니다. 검색이 잘리는 문제가 이 축을 크게 왜곡하지는
않았습니다. 대신 직업 구성은 뚜렷이 갈렸고, 더 큰 문제는 다른 데서 나왔습니다.
활성도 판정입니다. 최근 기록이 없으면 조용한 캐릭터로 봤는데, 명성이 낮은
캐릭터는 접속해도 기록에 남는 행동을 잘 하지 않습니다. 명성 방식 결과로
거꾸로 계산하면 낮은 구간의 조용한 비중이 크게 부풀었을 가능성이 나옵니다.

다만 이 역산은 1차 조사의 표본이 명성 쪽으로 치우치지 않았다는 가정 위에서
나온 값이고 그 가정이 바로 검증 대상이라, 원인을 표본 편향과 판정 오차로
갈라내지는 못했습니다. 리포트에도 그렇게 적었습니다.

### 실패한 접근도 남깁니다

처음에는 구간마다 위에서부터 차례로 받아 오려 했습니다. 그런데 명성 범위 조회는
한 번에 명성 폭 2,000만큼만 돌려줍니다. 구간이 그보다 훨씬 넓어서 받아 온
캐릭터가 구간 맨 위쪽 0.1에서 0.7% 폭에만 쌓였고 구간을 대표하지 못했습니다.
일정한 간격으로 훑는 방식으로 바꿔 해결했습니다.
...
python scripts/fame_survey2.py    # 명성 축 계통표집 검증 조사 (호출 947회, 약 5분)
python scripts/fame_analyze.py    # 1차 조사와 대조 분석
python scripts/build_verification.py  # 리포트가 읽을 대조표 생성
