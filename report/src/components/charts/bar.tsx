      ...
      {data.map((d, i) => {
        ...
        if (isHorizontal) {
          ...
          if (stacked && stackOffsets) {
            ...
            x = scale(offset) ?? 0;
            // `valuePos` is already this segment's pixel length, because the
            // horizontal value scale starts at 0 (range `[0, innerWidth]`).
            // Subtracting the stack offset here would shrink every segment
            // after the first and go negative once the offset passes the
            // segment's own value.
            barW = valuePos;
